    ...
    public class MonitoringJob
    {
        ...
        private async ValueTask BidAskCallback(IMyServiceBusMessage message, string topic)
        {
            try
            {
                ...
                if (lasted > 0 && lasted != message.Id - 1)
                {
                    ...
                    await _botApiClient.SendTextMessageAsync(Program.Settings.ChatId,
                        $"{Program.Settings.Name}] Wrong ID, Topic {topic}. Receive Id = {message.Id}, but lasted = {lasted}");
                }
            ...
            {
                Console.WriteLine($"Exception on single handler for {topic}\n{e}");
            }
        }
        ...
        private async ValueTask BidAskBatchCallback(IReadOnlyList<IMyServiceBusMessage> messages, string topic)
        {
            try
            {
                ...
                {
                    Console.WriteLine(
                        $"Wrong ID, Topic {topic}. Receive min Id = {min}, max = {max}, but lasted = {lasted}");
                    await _botApiClient.SendTextMessageAsync(Program.Settings.ChatId,
                        $"[{Program.Settings.Name}] Wrong ID, Topic {topic}. Receive min Id = {min}, max = {max}, but lasted = {lasted}");
                }

                if (messages.Count != max - min + 1)
                {
                    Console.WriteLine(
                        $"Miss messages in batch, Topic {topic}. Receive min Id = {min}, max = {max}, but count = {messages.Count}");
                    await _botApiClient.SendTextMessageAsync(Program.Settings.ChatId,
                        $"[{Program.Settings.Name}] Miss messages in batch, Topic {topic}. Receive min Id = {min}, max = {max}, but count = {messages.Count}");
                }
            ...
            {
                Console.WriteLine($"Exception on batch handler for {topic}\n{e}");
            }
        }
